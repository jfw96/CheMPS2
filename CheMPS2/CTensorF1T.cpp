...
CheMPS2::CTensorF1T::CTensorF1T( const int boundary_index, const int Idiff,
...
CheMPS2::CTensorF1T::~CTensorF1T() {}

void CheMPS2::CTensorF1T::makenew( CTensorT * denTup, CTensorT * denTdown, CTensorO * previous, dcomplex * workmem ) {
   clear();
...
void CheMPS2::CTensorF1T::makenew( CTensorLT * denL, CTensorT * denTup, CTensorT * denTdown, dcomplex * workmem ) {
   clear();
...
void CheMPS2::CTensorF1T::makenewRight( const int ikappa, CTensorT * denTup, CTensorT * denTdown, CTensorO * previous, dcomplex * workmem ) {
         ...
         if ( ( TwoSL >= 0 ) && ( abs( TwoSRD - TwoSL ) < 2 ) ) {
            ...
            if ( ( dimLU > 0 ) && ( dimLD > 0 ) ) {
               if ( previous == NULL ) {
                  ...
                  int fase       = ( ( ( ( TwoSL + TwoSRU + 3 ) / 2 ) % 2 ) != 0 ) ? -1 : 1;
                  dcomplex alpha = fase * sqrt( 3.0 * ( TwoSRD + 1 ) ) * Wigner::wigner6j( 1, 1, 2, TwoSRD, TwoSRU, TwoSL );
...
void CheMPS2::CTensorF1T::makenewLeft( const int ikappa, CTensorT * denTup, CTensorT * denTdown, CTensorO * previous, dcomplex * workmem ) {
         ...
         if ( ( TwoSR >= 0 ) && ( abs( TwoSLD - TwoSR ) < 2 ) ) {
            ...
            if ( ( dimRU > 0 ) && ( dimRD > 0 ) ) {
               if ( previous == NULL ) {
                  ...
                  int fase       = ( ( ( ( TwoSLU + TwoSR + 1 ) / 2 ) % 2 ) != 0 ) ? -1 : 1;
                  dcomplex alpha = fase * sqrt( 3.0 / ( TwoSLD + 1.0 ) ) * ( TwoSR + 1 ) *
                                   Wigner::wigner6j( 1, 1, 2, TwoSLD, TwoSLU, TwoSR );
                  ...
                  int fase       = ( ( ( ( TwoSLU + TwoSR + 1 ) / 2 ) % 2 ) != 0 ) ? -1 : 1;
                  dcomplex alpha = fase * sqrt( 3.0 / ( TwoSLD + 1.0 ) ) * ( TwoSR + 1 ) * Wigner::wigner6j( 1, 1, 2, TwoSLD, TwoSLU, TwoSR );
...
void CheMPS2::CTensorF1T::makenewRight( const int ikappa, CTensorLT * denL, CTensorT * denTup, CTensorT * denTdown, dcomplex * workmem ) {
   const int NRU    = sector_nelec_up[ ikappa ];
   const int TwoSRU = sector_spin_up[ ikappa ];
   const int IRU    = sector_irrep_up[ ikappa ];
   ...
   if ( dimRU > 0 && dimRD > 0 ) {
      for ( int geval = 0; geval < 4; geval++ ) {
         ...
         if ( ( dimLU > 0 ) && ( dimLD > 0 ) && ( abs( TwoSLU - TwoSLD ) < 2 ) ) {
            ...
            dcomplex alpha;
            if ( geval <= 1 ) {
               int fase = ( ( ( ( TwoSLD + TwoSRU + 3 ) / 2 ) % 2 ) != 0 ) ? -1 : 1;
...
void CheMPS2::CTensorF1T::makenewLeft( const int ikappa, CTensorLT * denL, CTensorT * denTup, CTensorT * denTdown, dcomplex * workmem ) {
   const int NLU    = sector_nelec_up[ ikappa ];
   const int TwoSLU = sector_spin_up[ ikappa ];
   const int ILU    = sector_irrep_up[ ikappa ];
      ...
      for ( int geval = 0; geval < 4; geval++ ) {
         ...
         if ( ( dimRU > 0 ) && ( dimRD > 0 ) && ( abs( TwoSRD - TwoSRU ) < 2 ) ) {
            ...
            if ( geval <= 1 ) {
               ...
               alpha    = fase * sqrt( 3.0 / ( TwoSLD + 1.0 ) ) * ( TwoSRU + 1 ) * Wigner::wigner6j( 1, 1, 2, TwoSLD, TwoSLU, TwoSRU );
            } else {
               int fase = ( ( ( TwoSLD ) % 2 ) != 0 ) ? -1 : 1;
               alpha    = fase * sqrt( 3.0 * ( TwoSRD + 1.0 ) * ( TwoSLU + 1.0 ) / ( TwoSLD + 1.0 ) ) * Wigner::wigner6j( 1, 1, 2, TwoSLD, TwoSLU, TwoSRD );
            }
            ...
            alpha = 1.0;
            beta  = 1.0; // add
            zgemm_( &notrans, &cotrans, &dimLU, &dimLD, &dimRD, &alpha, workmem, &dimLU, BlockTdown, &dimLD, &beta,
                    storage + kappa2index[ ikappa ], &dimLU );
         }
      }
   }
}
