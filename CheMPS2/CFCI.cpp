...
void CheMPS2::CFCI::StartupCountersVsBitstrings(){
...
void CheMPS2::CFCI::StartupLookupTables(){
...
void CheMPS2::CFCI::StartupIrrepCenter(){
...
int CheMPS2::CFCI::getUpIrrepOfCounter(const int irrep_center, const unsigned int counter) const{
...
void CheMPS2::CFCI::getBitsOfCounter(const int irrep_center, const unsigned int counter, int * bits_up, int * bits_down) const{
...
void CheMPS2::CFCI::setFCIcoeff( const int * bits_up, const int * bits_down, dcomplex value, dcomplex * vector) const{
   ...
   assert( counter_up != -1 );
   assert( counter_down != -1 );
   
   vector[ irrep_center_jumps[ 0 ][ irrep_up ] + counter_up + numPerIrrep_up[ irrep_up ] * counter_down ] = value;
...
void CheMPS2::CFCI::excite_alpha_omp( const unsigned int dim_new_up, const unsigned int dim_old_up, const unsigned int dim_down, dcomplex * origin, dcomplex * result, int * signmap, int * countmap ){
   ...
   for ( unsigned int cnt_new_up = 0; cnt_new_up < dim_new_up; cnt_new_up++ ){
      ...
      if ( sign_up != 0 ){
         ...
         for ( unsigned int cnt_down = 0; cnt_down < dim_down; cnt_down++ ){
            result[ cnt_new_up + dim_new_up * cnt_down ] += dcomplex( sign_up, 0 ) * origin[ cnt_old_up + dim_old_up * cnt_down ];
         }
      }
   }
...
void CheMPS2::CFCI::excite_beta_omp( const unsigned int dim_up, const unsigned int dim_new_down, dcomplex * origin, dcomplex * result, int * signmap, int * countmap ){
   ...
   for ( unsigned int cnt_new_down = 0; cnt_new_down < dim_new_down; cnt_new_down++ ){
      ...
      if ( sign_down != 0 ){
         ...
         for ( unsigned int cnt_up = 0; cnt_up < dim_up; cnt_up++ ){
            result[ cnt_up + dim_up * cnt_new_down ] += dcomplex( sign_down, 0 ) * origin[ cnt_up + dim_up * cnt_old_down ];
         }
      }
   }
...
void CheMPS2::CFCI::excite_alpha_first( const unsigned int dim_new_up, const unsigned int dim_old_up, const unsigned int start_down, const unsigned int stop_down, dcomplex * origin, dcomplex * result, int * signmap, int * countmap ){
   ...
   for ( unsigned int cnt_new_up = 0; cnt_new_up < dim_new_up; cnt_new_up++ ){
      ...
      if ( sign_up != 0 ){
         ...
         for ( unsigned int cnt_down = start_down; cnt_down < stop_down; cnt_down++ ){
            result[ cnt_new_up + dim_new_up * ( cnt_down - start_down ) ] += dcomplex( sign_up, 0 ) * origin[ cnt_old_up + dim_old_up * cnt_down ];
         }
      }
   }
...
void CheMPS2::CFCI::excite_beta_first( const unsigned int dim_up, const unsigned int start_down, const unsigned int stop_down, dcomplex * origin, dcomplex * result, int * signmap, int * countmap ){
   ...
   for ( unsigned int cnt_new_down = start_down; cnt_new_down < stop_down; cnt_new_down++ ){
      ...
      if ( sign_down != 0 ){
         const int cnt_old_down = countmap[ cnt_new_down ];
         for ( unsigned int cnt_up = 0; cnt_up < dim_up; cnt_up++ ){
            result[ cnt_up + dim_up * ( cnt_new_down - start_down ) ] += dcomplex( sign_down, 0 ) * origin[ cnt_up + dim_up * cnt_old_down ];
         }
      }
   }
...
void CheMPS2::CFCI::excite_alpha_second_omp( const unsigned int dim_new_up, const unsigned int dim_old_up, const unsigned int start_down, const unsigned int stop_down, dcomplex * origin, dcomplex * result, int * signmap, int * countmap ){
   ...
   for ( unsigned int cnt_old_up = 0; cnt_old_up < dim_old_up; cnt_old_up++ ){
      ...
      if ( sign_up != 0 ){ // Required for thread safety
         ...
         for ( unsigned int cnt_down = start_down; cnt_down < stop_down; cnt_down++ ){
            result[ cnt_new_up + dim_new_up * cnt_down ] += dcomplex( sign_up, 0 ) * origin[ cnt_old_up + dim_old_up * ( cnt_down - start_down ) ];
         }
      }
   }
...
void CheMPS2::CFCI::excite_beta_second_omp( const unsigned int dim_up, const unsigned int start_down, const unsigned int stop_down, dcomplex * origin, dcomplex * result, int * signmap, int * countmap ){
   ...
   for ( unsigned int cnt_old_down = start_down; cnt_old_down < stop_down; cnt_old_down++ ){
      ...
      if ( sign_down != 0 ){ // Required for thread safety
         ...
         for ( unsigned int cnt_up = 0; cnt_up < dim_up; cnt_up++ ){
            result[ cnt_up + dim_up * cnt_new_down ] += dcomplex( sign_down, 0 ) * origin[ cnt_up + dim_up * ( cnt_old_down - start_down ) ];
         }
      }
   }
...
void CheMPS2::CFCI::matvec( dcomplex * input, dcomplex * output ) const{
            ...
            for ( unsigned int block = 0; block < num_block_beta; block++ ){
               ...
               if ( size_center > 0 ){
                  ...
                  for ( unsigned int pair = 0; pair < num_pairs; pair++ ){
                     dcomplex * target_space   = HXVworkbig1 + size_center * pair;
                  ...
                  if ( irrep_center == 0 ){
                     ...
                     int kdim = num_pairs;
                     int ndim = 1;
                     dcomplex * target = output + zero_jumps[ irrep_center_up ] + dim_center_up * start_center_down;
                     zgemm_( &notrans, &notrans, &mdim, &ndim, &kdim, &one, HXVworkbig1, &mdim, HXVworksmall, &kdim, &one, target, &mdim );
                  }
                  ...
                  // Now build workbig2[ veccounter + size_center * new_pair ] = 0.5 * ( new_pair | old_pair ) * workbig1[ veccounter + size_center * old_pair ]
                  {
                     ...
                     int kdim = num_pairs;
                     int ndim = num_pairs;
                     zgemm_( &notrans, &notrans, &mdim, &ndim, &kdim, &one, HXVworkbig1, &mdim, HXVworksmall, &kdim, &set, HXVworkbig2, &mdim );
                  }

                  // Finally do output <-- E_{i<=j} + (1 - delta_{i==j}) E_{j>i} workbig2[ veccounter + size_center * pair ]
                  for ( unsigned int pair = 0; pair < num_pairs; pair++ ){
                     dcomplex * origin_space   = HXVworkbig2 + size_center * pair;
                     const unsigned int crea = center_crea_orb[ pair ];
                     const unsigned int anni = center_anni_orb[ pair ];
                     const int irrep_excited = Irreps::directProd( getOrb2Irrep( crea ), getOrb2Irrep( anni ) );
                     const int irrep_zero_up = Irreps::directProd( irrep_excited, irrep_center_up );
...
void CheMPS2::CFCI::apply_excitation( dcomplex * orig_vector, dcomplex * result_vector, const int crea, const int anni, const int orig_target_irrep ) const{
...
dcomplex CheMPS2::CFCI::Fill2RDM(dcomplex * vector, dcomplex * two_rdm) const{
   ...
   dcomplex * workspace1 = new dcomplex[ max_length  ];
   dcomplex * workspace2 = new dcomplex[ orig_length ];
   ...
   for ( unsigned int anni1 = 0; anni1 < L; anni1++ ){ // anni1 = l
      for ( unsigned int crea1 = anni1; crea1 < L; crea1++ ){ // crea1 = j >= l
         ...
         if ( irrep_center1 == 0 ){
            const dcomplex value = FCIddot( orig_length, workspace1, vector ); // < E_{crea1,anni1} >
         ...
         for ( unsigned int crea2 = anni1; crea2 < L; crea2++ ){ // crea2 = i >= l
            for ( unsigned int anni2 = anni1; anni2 < L; anni2++ ){ // anni2 = k >= l
               ...
               if ( irrep_center2 == irrep_center1 ){
                  ...
                  const dcomplex value = FCIddot( orig_length, workspace2, vector ); // < E_{crea2,anni2} E_{crea1,anni1} >
   ...
   dcomplex FCIenergy = getEconst();
   for ( unsigned int orb1 = 0; orb1 < L; orb1++ ){
      for ( unsigned int orb2 = 0; orb2 < L; orb2++ ){
         dcomplex tempvar = 0.0;
         dcomplex tempvar2 = 0.0;
         for ( unsigned int orb3 = 0; orb3 < L; orb3++ ){
...
void CheMPS2::CFCI::FCIdcopy(const unsigned int vecLength, dcomplex * origin, dcomplex * target){

   int length = vecLength; // Checked "assert( max_integer >= maxVecLength );" at FCI::StartupIrrepCenter()
   int inc = 1;
   zcopy_( &length , origin , &inc , target , &inc );

}

dcomplex CheMPS2::CFCI::FCIddot(const unsigned int vecLength, dcomplex * vec1, dcomplex * vec2){
...
dcomplex CheMPS2::CFCI::FCIaHb(const unsigned int vecLength, dcomplex * vec1, dcomplex * vec2){
...
void CheMPS2::CFCI::FCIdaxpy(const unsigned int vecLength, const dcomplex alpha, dcomplex * vec_x, dcomplex * vec_y){

   dcomplex factor = alpha;
   int length = vecLength; // Checked "assert( max_integer >= maxVecLength );" at FCI::StartupIrrepCenter()
   int inc = 1;
   zaxpy_( &length , &factor , vec_x , &inc , vec_y , &inc );

}

void CheMPS2::CFCI::FCIdscal(const unsigned int vecLength, const dcomplex alpha, dcomplex * vec){

   dcomplex factor = alpha;
   int length = vecLength; // Checked "assert( max_integer >= maxVecLength );" at FCI::StartupIrrepCenter()
   int inc = 1;
   zscal_( &length , &factor , vec , &inc );

}

void CheMPS2::CFCI::ClearVector(const unsigned int vecLength, dcomplex * vec){

   for ( unsigned int cnt = 0; cnt < vecLength; cnt++ ){ vec[cnt] = 0.0; }

}

void CheMPS2::CFCI::FillRandom(const unsigned int vecLength, dcomplex * vec){
...
void CheMPS2::CFCI::ArnoldiTimeStep( double timeStep, const bool dobackwards, unsigned int krylovSize, dcomplex * input, dcomplex * output){
   ...
   for ( int kry = 1; kry < krylovSpaceDimension; kry++ ) {
      ...
      ClearVector( veclength, newKrylov );
...
void CheMPS2::CFCI::ActWithNumberOperator(const unsigned int orbIndex, dcomplex * resultVector, dcomplex * sourceVector) const{
   ...
   for (unsigned int counter = 0; counter < vecLength; counter++){
      ...
      resultVector[ counter ] = dcomplex( bits_up[ orbIndex ] + bits_down[ orbIndex ], 0 ) * sourceVector[ counter ];
   }
...
void CheMPS2::CFCI::getFCITensor( dcomplex * state, 
                                  std::vector< std::vector< int > >& alphasOut,
                                  std::vector< std::vector< int > >& betasOut,
                                  std::vector< double >& coefsRealOut,
                                  std::vector< double >& coefsImagOut){
   ...
   recusion( state, L, alphas, betas, alphasOut, betasOut, coefsRealOut, coefsImagOut );
}

void CheMPS2::CFCI::recusion( dcomplex * state, int L,
                              std::vector< int > alphas, std::vector< int > betas,
                              std::vector< std::vector< int > > & alphasOut,
                              std::vector< std::vector< int > > & betasOut,
                              std::vector< double > & coefsRealOut,
                              std::vector< double > & coefsImagOut ) {
   ...
   if ( alphas.size() == L && betas.size() == L ) {
      if ( sumAlpha + sumBeta == Nel_up + Nel_down ) {
         ...
         coefsImagOut.push_back( std::imag( getFCIcoeff( &alphas[ 0 ], &betas[ 0 ], state ) ) );
      }
   } else if ( alphas.size() == L ) {
      std::vector< int > caseA = betas;
      caseA.push_back( 0 );
      recusion( state, L, alphas, caseA, alphasOut, betasOut, coefsRealOut, coefsImagOut );
      std::vector< int > caseB = betas;
      caseB.push_back( 1 );
      recusion( state, L, alphas, caseB, alphasOut, betasOut, coefsRealOut, coefsImagOut );
   } else {
      std::vector< int > caseA = alphas;
      caseA.push_back( 0 );
      recusion( state, L, caseA, betas, alphasOut, betasOut, coefsRealOut, coefsImagOut );
      std::vector< int > caseB = alphas;
      caseB.push_back( 1 );
      recusion( state, L, caseB, betas, alphasOut, betasOut, coefsRealOut, coefsImagOut );
   }
}
