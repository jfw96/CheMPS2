...
void CheMPS2::CHeffNS_1S::addDiagram4B1and4B2spin0( const int ikappa, dcomplex * memHeff, CTensorT * in, CTensorT * out, CTensorOperator *** Aleft, CTensorOperator *** ATleft, CTensorL ** Lright, CTensorLT ** LTright, dcomplex * temp ) {
   ...
   if ( N1 == 1 ) {
      ...
      for ( int l_index = theindex + 1; l_index < Prob->gL(); l_index++ ) {
...
#endif
         {
            int ILdown    = Irreps::directProd( IL, Aleft[ l_index - theindex ][ 0 ]->get_irrep() );
            int IRdown    = Irreps::directProd( IR, bk_up->gIrrep( l_index ) );
            int memSkappa = in->gKappa( NL - 2, TwoSL, ILdown, NR - 1, TwoSL, IRdown );
            ...
            if ( memSkappa != -1 ) {
               ...
               dcomplex * Lblock = LTright[ l_index - theindex - 1 ]->gStorage( NR, TwoSR, IR, NR - 1, TwoSL, IRdown );
               dcomplex alpha    = 1.0;
               dcomplex beta     = 0.0; //set
               zgemm_( &notrans, &cotrans, &dimLdown, &dimRup, &dimRdown, &alpha, in->gStorage() + in->gKappa2index( memSkappa ), &dimLdown, Lblock, &dimRup, &beta, temp, &dimLdown );
   ...
   if ( N1 == 1 ) {
      ...
      int fase              = phase( 3 * TwoSL + TwoSR + 3 );
      const dcomplex factor = fase * sqrt( 0.5 ) * sqrt( ( TwoSL + 1.0 ) / ( TwoSR + 1.0 ) );
      ...
      for ( int l_index = theindex + 1; l_index < Prob->gL(); l_index++ ) {
...
#endif
         {
            ...
            if ( memSkappa != -1 ) {
               ...
               dcomplex * Lblock = Lright[ l_index - theindex - 1 ]->gStorage( NR, TwoSR, IR, NR + 1, TwoSL, IRdown );
               dcomplex alpha    = 1.0;
               dcomplex beta     = 0.0; //set
               zgemm_( &notrans, &cotrans, &dimLdown, &dimRup, &dimRdown, &alpha, in->gStorage() + in->gKappa2index( memSkappa ), &dimLdown, Lblock, &dimRup, &beta, temp, &dimLdown );
   ...
   if ( N1 == 2 ) {
      ...
      for ( int TwoSRdown = TwoSR - 1; TwoSRdown <= TwoSR + 1; TwoSRdown += 2 ) {
         ...
         if ( TwoSRdown >= 0 ) {
            ...
            int fase              = phase( 2 + 2 * TwoSRdown + 2 * TwoSL );
            const dcomplex factor = fase * sqrt( 0.5 ) * ( ( TwoSRdown + 1.0 ) / ( TwoSR + 1.0 ) );
...
void CheMPS2::CHeffNS_1S::addDiagram4B3and4B4spin0( const int ikappa, dcomplex * memHeff, CTensorT * in, CTensorT * out, CTensorOperator *** Cleft, CTensorOperator *** CTleft, CTensorL ** Lright, CTensorLT ** LTright, dcomplex * temp ) {
   ...
   if ( N1 == 1 ) {
      ...
      int fase              = phase( 2 + 2 * TwoSL + 2 * TwoSR );
      const dcomplex factor = fase * sqrt( 0.5 );
      ...
      for ( int l_index = theindex + 1; l_index < Prob->gL(); l_index++ ) {
...
#endif
         {
            ...
            int IRdown    = Irreps::directProd( IR, bk_up->gIrrep( l_index ) );
            int memSkappa = in->gKappa( NL, TwoSL, ILdown, NR - 1, TwoSL, IRdown );
   ...
   if ( N1 == 2 ) {
      ...
      for ( int TwoSRdown = TwoSR - 1; TwoSRdown <= TwoSR + 1; TwoSRdown += 2 ) {
         ...
         if ( TwoSRdown >= 0 ) {
            ...
            int fase              = phase( 3 + 3 * TwoSR + TwoSRdown );
            const dcomplex factor = fase * sqrt( 0.5 ) * sqrt( ( TwoSRdown + 1.0 ) / ( TwoSR + 1.0 ) );
   ...
   if ( N1 == 0 ) {
      ...
      for ( int TwoSRdown = TwoSR - 1; TwoSRdown <= TwoSR + 1; TwoSRdown += 2 ) {
         ...
         if ( TwoSRdown >= 0 ) {
            ...
            int fase              = phase( 2 + 2 * TwoSRdown + TwoSL + TwoSR );
            const dcomplex factor = fase * sqrt( 0.5 ) * ( TwoSRdown + 1.0 ) / ( TwoSR + 1.0 );
   ...
   if ( N1 == 1 ) {
      ...
      int fase              = phase( 3 + 3 * TwoSL + TwoSR );
      const dcomplex factor = fase * sqrt( 0.5 ) * sqrt( ( TwoSL + 1.0 ) / ( TwoSR + 1.0 ) );
...
void CheMPS2::CHeffNS_1S::addDiagram4B3and4B4spin1( const int ikappa, dcomplex * memHeff, CTensorT * in, CTensorT * out, CTensorOperator *** Dleft, CTensorOperator *** DTleft, CTensorL ** Lright, CTensorLT ** LTright, dcomplex * temp ) {
   ...
   if ( N1 == 1 ) {
      ...
      for ( int TwoSRdown = TwoSR - 1; TwoSRdown <= TwoSR + 1; TwoSRdown += 2 ) {
         for ( int TwoSLdown = TwoSL - 2; TwoSLdown <= TwoSL + 2; TwoSLdown += 2 ) {
            if ( ( abs( TwoSLdown - TwoSRdown ) == 0 ) && ( TwoSLdown >= 0 ) && ( TwoSRdown >= 0 ) ) {

               int fase              = phase( 2 * TwoSL + 5 + TwoSR - TwoSRdown );
               const dcomplex factor = fase * sqrt( 3.0 ) * sqrt( TwoSRdown + 1.0 ) * Wigner::wigner6j( 2, TwoSL, TwoSLdown, TwoSR, 1, 1 );
   ...
   if ( N1 == 2 ) {
      ...
      for ( int TwoSRdown = TwoSR - 1; TwoSRdown <= TwoSR + 1; TwoSRdown += 2 ) {
         for ( int TwoSLdown = TwoSL - 2; TwoSLdown <= TwoSL + 2; TwoSLdown += 2 ) {
            ...
            int TwoJstart = ( ( TwoSLdown != TwoSRdown ) || ( TwoS2 == 0 ) ) ? TwoS2 + 1 : 0;
            for ( int TwoJdown = TwoJstart; TwoJdown <= TwoS2 + 1; TwoJdown += 2 ) {
               if ( ( ( TwoJdown == TwoS ) && abs( TwoSLdown - TwoSRdown ) <= TwoJdown ) && ( TwoSLdown >= 0 ) && ( TwoSRdown >= 0 ) ) {

                  int fase              = phase( TwoSLdown + TwoSL + 4 );
                  const dcomplex factor = fase * sqrt( 3.0 ) * sqrt( TwoSRdown + 1.0 ) * Wigner::wigner6j( 2, TwoSLdown, TwoSL, TwoSR, 1, 1 );
         ...
         for ( int TwoSLdown = TwoSL - 2; TwoSLdown <= TwoSL + 2; TwoSLdown += 2 ) {
            ...
            for ( int TwoJdown = TwoJstart; TwoJdown <= TwoS2 + 1; TwoJdown += 2 ) {
               if ( ( ( TwoJdown == TwoS ) && abs( TwoSLdown - TwoSRdown ) <= TwoJdown ) && ( TwoSLdown >= 0 ) && ( TwoSRdown >= 0 ) ) {
                  ...
                  int fase              = phase( 2 * TwoSLdown + 5 + TwoSRdown - TwoSR );
                  const dcomplex factor = fase * sqrt( 3.0 ) * sqrt( ( TwoSLdown + 1.0 ) / ( ( TwoSR + 1.0 ) * ( TwoSL + 1 ) ) ) * ( TwoSRdown + 1.0 ) * Wigner::wigner6j( 2, TwoSLdown, TwoSL, TwoSR, 1, 1 );
      ...
      for ( int TwoSRdown = TwoSR - 1; TwoSRdown <= TwoSR + 1; TwoSRdown += 2 ) {
         for ( int TwoSLdown = TwoSL - 2; TwoSLdown <= TwoSL + 2; TwoSLdown += 2 ) {
            if ( ( abs( TwoSLdown - TwoSRdown ) <= 0 ) && ( TwoSLdown >= 0 ) && ( TwoSRdown >= 0 ) ) {
               ...
               int fase              = phase( TwoSL + TwoSLdown + 4 );
               const dcomplex factor = fase * sqrt( 3.0 ) * sqrt( 1.0 / ( TwoSR + 1.0 ) ) * ( TwoSRdown + 1.0 ) * Wigner::wigner6j( 2, TwoSL, TwoSLdown, TwoSR, 1, 1 );
            ...
            for ( int TwoJdown = TwoJstart; TwoJdown <= TwoS2 + 1; TwoJdown += 2 ) {
               if ( ( abs( TwoSLdown - TwoSRdown ) <= TwoJdown ) && ( TwoSLdown >= 0 ) && ( TwoSRdown >= 0 ) ) {
                  ...
                  if ( TwoJ == TwoJdown ) {
                     ...
                     factor2 = fase * sqrt( ( TwoSLdown + 1.0 ) * ( TwoSRdown + 1.0 ) ) * Wigner::wigner6j( TwoSL, TwoSR, TwoJ, TwoSRdown, TwoSLdown, 1 );
                  }
