...
CheMPS2::CTensorQT::CTensorQT( const int boundary_index, const int Idiff,
...
CheMPS2::CTensorQT::~CTensorQT() {}

void CheMPS2::CTensorQT::AddTermSimple( CTensorT * denTup, CTensorT * denTdown, CTensorO * previous, dcomplex * workmem ) {
...
void CheMPS2::CTensorQT::AddTermSimpleRight( CTensorT * denTup, CTensorT * denTdown, CTensorO * previous, dcomplex * workmem ) {
   ...
   for ( int ikappa = 0; ikappa < nKappa; ikappa++ ) {
      ...
      if ( ( dimLU > 0 ) && ( dimLD > 0 ) ) {
         if ( previous == NULL ) {
            ...
            int fase       = ( ( ( ( TwoSRU + 1 - TwoSRD ) / 2 ) % 2 ) != 0 ) ? -1 : 1;
            dcomplex alpha = fase * mxElement * sqrt( ( TwoSRD + 1.0 ) / ( TwoSRU + 1.0 ) );
...
void CheMPS2::CTensorQT::AddTermSimpleLeft( CTensorT * denTup, CTensorT * denTdown, CTensorO * previous, dcomplex * workmem ) {
   ...
   for ( int ikappa = 0; ikappa < nKappa; ikappa++ ) {
      ...
      if ( ( dimRU > 0 ) && ( dimRD > 0 ) ) {
         if ( previous == NULL ) {
            ...
            int fase       = ( ( ( ( TwoSLD + 1 - TwoSLU ) / 2 ) % 2 ) != 0 ) ? -1 : 1;
            dcomplex alpha = fase * mxElement * sqrt( ( TwoSLD + 1.0 ) / ( TwoSLU + 1.0 ) );
            dcomplex beta  = 1.0; // add
...
void CheMPS2::CTensorQT::AddTermsL( CTensorL ** Ltensors, CTensorLT ** LtensorsT, CTensorT * denTup, CTensorT * denTdown, dcomplex * workmem, dcomplex * workmem2 ) {
   if ( moving_right ) {
...
void CheMPS2::CTensorQT::AddTermsLRight( CTensorL ** Ltensors, CTensorLT ** LtensorsT, CTensorT * denTup, CTensorT * denTdown, dcomplex * workmem, dcomplex * workmem2 ) {
   bool OneToAdd = false;
   ...
   if ( OneToAdd ) {
      for ( int ikappa = 0; ikappa < nKappa; ikappa++ ) {
         ...
         if ( ( dimLU > 0 ) && ( dimLD > 0 ) ) {
            ...
            int fase       = ( ( ( ( TwoSRU + 1 - TwoSRD ) / 2 ) % 2 ) != 0 ) ? -1 : 1;
            dcomplex alpha = fase * sqrt( ( TwoSRD + 1.0 ) / ( TwoSRU + 1.0 ) );
         ...
         if ( ( dimLU > 0 ) && ( dimLD > 0 ) ) {
            ...
            for ( int loca = 0; loca < index - 1; loca++ ) {
               if ( LtensorsT[ index - 2 - loca ]->get_irrep() == n_irrep ) {
                  ...
                  dcomplex alpha =
                      2 * Prob->gMxElement( loca, index - 1, site, index - 1 ) -
                      Prob->gMxElement( loca, index - 1, index - 1, site );
                  int inc = 1;
                  zaxpy_( &dimLUxLD, &alpha, BlockL, &inc, workmem, &inc );
               }
            }
               ...
               if ( ( TwoSLD >= 0 ) && ( TwoSLU >= 0 ) && ( abs( TwoSLD - TwoSLU ) < 2 ) ) {
                  ...
                  if ( ( dimLU > 0 ) && ( dimLD > 0 ) ) {
                     ...
                     dcomplex factor =
                         ...
                         Wigner::wigner6j( TwoSRD, TwoSRU, 1, TwoSLU, TwoSLD, 1 );
                     ...
                     for ( int loca = 0; loca < index - 1; loca++ ) {
                        if ( LtensorsT[ index - 2 - loca ]->get_irrep() == n_irrep ) {
                           dcomplex * BlockL = LtensorsT[ index - 2 - loca ]->gStorage( NLU, TwoSLU, ILU, NLD, TwoSLD, ILD );
...
void CheMPS2::CTensorQT::AddTermsLLeft( CTensorL ** Ltensors, CTensorLT ** LtensorsT, CTensorT * denTup, CTensorT * denTdown, dcomplex * workmem, dcomplex * workmem2 ) {
   bool OneToAdd = false;
   ...
   if ( OneToAdd ) {
      for ( int ikappa = 0; ikappa < nKappa; ikappa++ ) {
         ...
         if ( ( dimRU > 0 ) && ( dimRD > 0 ) ) {
            ...
            for ( int loca = index + 1; loca < Prob->gL(); loca++ ) {
               if ( Ltensors[ loca - index - 1 ]->get_irrep() == n_irrep ) {
                  dcomplex * BlockL = Ltensors[ loca - index - 1 ]->gStorage(
            ...
            int fase       = ( ( ( ( TwoSLD + 1 - TwoSLU ) / 2 ) % 2 ) != 0 ) ? -1 : 1;
            dcomplex alpha = fase * sqrt( ( TwoSLD + 1.0 ) / ( TwoSLU + 1.0 ) );
            dcomplex beta  = 0.0; // set
               ...
               if ( ( TwoSRD >= 0 ) && ( TwoSRU >= 0 ) && ( abs( TwoSRD - TwoSRU ) < 2 ) ) {
                  ...
                  if ( ( dimRU > 0 ) && ( dimRD > 0 ) ) {
                     ...
                     for ( int loca = index + 1; loca < Prob->gL(); loca++ ) {
                        if ( LtensorsT[ loca - index - 1 ]->get_irrep() == n_irrep ) {
                           dcomplex * BlockL =
                               LtensorsT[ loca - index - 1 ]->gStorage( NRU, TwoSRU, IRU,
                                                                        NRD, TwoSRD, IRD );
                           dcomplex alpha =
                               factor1 * Prob->gMxElement( site, index, loca, index );
                           if ( TwoSRD == TwoSLU ) {
                              alpha +=
                                  factor2 * Prob->gMxElement( site, index, index, loca );
                           }
                           int inc = 1;
                           zaxpy_( &dimRUxRD, &alpha, BlockL, &inc, workmem, &inc );
                        }
...
void CheMPS2::CTensorQT::AddTermsAB( CTensorOperator * denA, CTensorOperator * denB, CTensorT * denTup,
                                     CTensorT * denTdown, dcomplex * workmem, dcomplex * workmem2 ) {
   if ( moving_right ) {
...
void CheMPS2::CTensorQT::AddTermsABRight( CTensorOperator * denA, CTensorOperator * denB, CTensorT * denTup,
                                          CTensorT * denTdown, dcomplex * workmem, dcomplex * workmem2 ) {
   ...
   for ( int ikappa = 0; ikappa < nKappa; ikappa++ ) {
      ...
      for ( int TwoSLD = TwoSRD - 1; TwoSLD <= TwoSRD + 1; TwoSLD += 2 ) {
         ...
         if ( ( dimLU > 0 ) && ( dimLD > 0 ) ) {
            ...
            if ( TwoSLD == TwoSRU ) {
               fase             = ( ( ( ( TwoSRU + 1 - TwoSRD ) / 2 ) % 2 ) != 0 ) ? -1 : 1;
               dcomplex factorA = fase * sqrt( 0.5 * ( TwoSRD + 1.0 ) / ( TwoSRU + 1.0 ) );
      ...
      // case 2
      NLU = NRU - 1;
      ILU = Irreps::directProd( IRU, bk_down->gIrrep( index - 1 ) );
      ...
      for ( TwoSLU = TwoSRU - 1; TwoSLU <= TwoSRU + 1; TwoSLU += 2 ) {
         ...
         if ( ( dimLU > 0 ) && ( dimLD > 0 ) ) {
            ...
            zgemm_( &cotrans, &notrans, &dimRU, &dimLD, &dimLU, &alpha,
                    BlockTup, &dimLU, mem, &dimLU, &beta, workmem2, &dimRU );
            alpha = 1.0;
            beta  = 1.0; // add
            zgemm_( &notrans, &notrans, &dimRU, &dimRD, &dimLD, &alpha,
                    workmem2, &dimRU, BlockTdo, &dimLD, &beta, storage + kappa2index[ ikappa ], &dimRU );
         }
      }
   }
}

void CheMPS2::CTensorQT::AddTermsABLeft( CTensorOperator * denA, CTensorOperator * denB, CTensorT * denTup,
                                         CTensorT * denTdown, dcomplex * workmem, dcomplex * workmem2 ) {
   ...
   for ( int ikappa = 0; ikappa < nKappa; ikappa++ ) {
      ...
      for ( int TwoSRU = TwoSLU - 1; TwoSRU <= TwoSLU + 1; TwoSRU += 2 ) {
         ...
         if ( ( dimRU > 0 ) && ( dimRD > 0 ) ) {
            ...
            if ( TwoSRU == TwoSLD ) {
               fase             = ( ( ( ( TwoSLD + 1 - TwoSLU ) / 2 ) % 2 ) != 0 ) ? -1 : 1;
               dcomplex factorA = fase * sqrt( 0.5 * ( TwoSLD + 1.0 ) / ( TwoSLU + 1.0 ) );
            ...
            zgemm_( &notrans, &notrans, &dimLU, &dimRD, &dimRU, &alpha,
                    BlockTup, &dimLU, mem, &dimRU, &beta, workmem2, &dimLU );

            alpha = 1.0;
            beta  = 1.0; // add
            zgemm_( &notrans, &cotrans, &dimLU, &dimLD, &dimRD, &alpha,
                    workmem2, &dimLU, BlockTdo, &dimLD, &beta, storage + kappa2index[ ikappa ], &dimLU );
         }
      }
...
void CheMPS2::CTensorQT::AddTermsCD( CTensorOperator * denC, CTensorOperator * denD, CTensorT * denTup,
                                     CTensorT * denTdown, dcomplex * workmem, dcomplex * workmem2 ) {
   if ( moving_right ) {
...
void CheMPS2::CTensorQT::AddTermsCDRight( CTensorOperator * denC, CTensorOperator * denD, CTensorT * denTup,
                                          CTensorT * denTdown, dcomplex * workmem, dcomplex * workmem2 ) {
   ...
   for ( int ikappa = 0; ikappa < nKappa; ikappa++ ) {
      ...
      for ( int TwoSLU = TwoSRU - 1; TwoSLU <= TwoSRU + 1; TwoSLU += 2 ) {
         ...
         if ( ( dimLU > 0 ) && ( dimLD > 0 ) ) {
            ...
            // first set to D
            int fase        = ( ( ( ( TwoSRD + TwoSRU + 1 ) / 2 ) % 2 ) != 0 ) ? -1 : 1;
            dcomplex factor = fase * sqrt( 3.0 * ( TwoSLU + 1 ) ) *
            ...
            dcomplex beta  = 0.0;
            zgemm_( &cotrans, &notrans, &dimRU, &dimLD, &dimLU, &alpha,
                    BlockTup, &dimLU, workmem, &dimLU, &beta, workmem2, &dimRU );
      ...
      for ( TwoSLD = TwoSRD - 1; TwoSLD <= TwoSRD + 1; TwoSLD += 2 ) {
         ...
         if ( ( dimLU > 0 ) && ( dimLD > 0 ) ) {
            ...
            dcomplex factor = fase * sqrt( 3.0 * ( TwoSRD + 1 ) ) * Wigner::wigner6j( 1, 2, 1, TwoSRU, TwoSRD, TwoSLD );
            ...
            if ( TwoSLD == TwoSRU ) {
               fase    = ( ( ( ( TwoSRU + 1 - TwoSRD ) / 2 ) % 2 ) != 0 ) ? -1 : 1;
               factor  = fase * sqrt( 0.5 * ( TwoSRD + 1.0 ) / ( TwoSRU + 1.0 ) );
               block   = denC->gStorage( NLU, TwoSLU, ILU, NLD, TwoSLD, ILD );
               int inc = 1;
               zaxpy_( &dimLUxLD, &factor, block, &inc, workmem, &inc );
            }
...
void CheMPS2::CTensorQT::AddTermsCDLeft( CTensorOperator * denC, CTensorOperator * denD, CTensorT * denTup,
                                         CTensorT * denTdown, dcomplex * workmem, dcomplex * workmem2 ) {
   ...
   for ( int ikappa = 0; ikappa < nKappa; ikappa++ ) {
      ...
      for ( int TwoSRD = TwoSLD - 1; TwoSRD <= TwoSLD + 1; TwoSRD += 2 ) {
         ...
         if ( ( dimRU > 0 ) && ( dimRD > 0 ) ) {
            ...
            // first set to D
            int fase        = ( ( ( ( TwoSLD + TwoSRD + 3 ) / 2 ) % 2 ) != 0 ) ? -1 : 1;
            dcomplex factor = fase * sqrt( 3.0 / ( TwoSLU + 1.0 ) ) * ( TwoSRD + 1 ) *
                              Wigner::wigner6j( 1, 1, 2, TwoSRD, TwoSLU, TwoSLD );
            ...
            dcomplex beta  = 0.0;
            zgemm_( &notrans, &notrans, &dimLU, &dimRD, &dimRU, &alpha,
                    BlockTup, &dimLU, workmem, &dimRU, &beta, workmem2, &dimLU );
      ...
      // case 2
      NRU = NLU + 1;
      IRU = Irreps::directProd( ILU, bk_down->gIrrep( index ) );
      ...
      for ( TwoSRU = TwoSLU - 1; TwoSRU <= TwoSLU + 1; TwoSRU += 2 ) {
         ...
         if ( ( dimRU > 0 ) && ( dimRD > 0 ) ) {
            ...
            // first set to D
            int fase        = ( ( ( TwoSRU + 1 ) % 2 ) != 0 ) ? -1 : 1;
            dcomplex factor = fase * sqrt( 3.0 * ( TwoSRU + 1.0 ) * ( TwoSLD + 1.0 ) /
                                           ( TwoSLU + 1.0 ) ) *
                              Wigner::wigner6j( 1, 1, 2, TwoSLD, TwoSRU, TwoSLU );
            ...
            if ( TwoSRU == TwoSLD ) {
               fase    = ( ( ( ( TwoSLD + 1 - TwoSLU ) / 2 ) % 2 ) != 0 ) ? -1 : 1;
               factor  = fase * sqrt( 0.5 * ( TwoSLD + 1.0 ) / ( TwoSLU + 1.0 ) );
               block   = denC->gStorage( NRU, TwoSRU, IRU, NRD, TwoSRD, IRD );
               int inc = 1;
               zaxpy_( &dimRUxRD, &factor, block, &inc, workmem, &inc );
            }
