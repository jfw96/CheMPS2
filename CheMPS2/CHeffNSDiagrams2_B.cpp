...
void CheMPS2::CHeffNS::addDiagram2a1spin0( const int ikappa, dcomplex * memS, dcomplex * memHeff, CSobject * denS,
         ...
         for ( int l_beta = l_alpha; l_beta < theindex; l_beta++ ) {
...
#endif
            {
               ...
               if ( memSkappa != -1 ) {
                  ...
                  dcomplex alpha = 1.0;
                  dcomplex beta  = 0.0;
                  zgemm_( &notrans, &notrans, &dimL, &dimRdown, &dimLdown, &alpha, BlockS0,
                          &dimL, memS + denS->gKappa2index( memSkappa ), &dimLdown, &beta, workspace, &dimL );

                  beta = 1.0;
                  zgemm_( &notrans, &cotrans, &dimL, &dimR, &dimRdown, &alpha, workspace,
                          &dimL, BlockA, &dimR, &beta, memHeff, &dimL );
               }
            }
         }
      }
         ...
         for ( int l_delta = l_gamma; l_delta < Prob->gL(); l_delta++ ) {
...
#endif
            {
               ...
               if ( memSkappa != -1 ) {
                  ...
                  zgemm_( &notrans, &notrans, &dimL, &dimRdown, &dimLdown, &alpha, BlockA,
                          &dimL, memS + denS->gKappa2index( memSkappa ), &dimLdown, &beta, workspace, &dimL );

                  beta = 1.0;
                  zgemm_( &notrans, &cotrans, &dimL, &dimR, &dimRdown, &alpha, workspace,
                          &dimL, BlockS0, &dimR, &beta, memHeff, &dimL );
               }
            }
         }
      }
...
void CheMPS2::CHeffNS::addDiagram2a2spin0( const int ikappa, dcomplex * memS, dcomplex * memHeff, CSobject * denS,
         ...
         for ( int l_beta = l_alpha; l_beta < theindex; l_beta++ ) {
...
#endif
            {
               ...
               if ( memSkappa != -1 ) {
                  ...
                  dcomplex * BlockS0 = S0tensors[ theindex - 1 ][ l_beta - l_alpha ][ theindex - 1 - l_beta ]
                                           ->gStorage( NL, TwoSL, IL, NL + 2, TwoSL, ILdown );
                  dcomplex * BlockA = Atensors[ theindex + 1 ][ l_beta - l_alpha ][ theindex + 1 - l_beta ]
                                          ->gStorage( NR, TwoSR, IR, NR + 2, TwoSR, IRdown );
...
void CheMPS2::CHeffNS::addDiagram2a1spin1( const int ikappa, dcomplex * memS, dcomplex * memHeff, CSobject * denS, CTensorOperator **** BTtensors, CTensorS1T **** S1Ttensors, dcomplex * workspace ) {
...
#endif
                     {
                        int ILdown = Irreps::directProd( IL, BTtensors[ theindex - 1 ][ l_delta - l_gamma ][ l_gamma - theindex ]
                                                                 ->get_irrep() );
                        int IRdown = Irreps::directProd( IR, S1Ttensors[ theindex + 1 ][ l_delta - l_gamma ][ l_gamma - theindex - 2 ]
                                                                 ->get_irrep() );
                        int memSkappa = denS->gKappa( NL - 2, TwoSLdown, ILdown, N1, N2, TwoJ, NR - 2, TwoSRdown, IRdown );
...
void CheMPS2::CHeffNS::addDiagram2a2spin1( const int ikappa, dcomplex * memS, dcomplex * memHeff, CSobject * denS, CTensorOperator **** Btensors, CTensorS1 **** S1tensors, dcomplex * workspace ) {
...
#endif
                     {
                        int ILdown = Irreps::directProd( IL, S1tensors[ theindex - 1 ][ l_beta - l_alpha ][ theindex - 1 - l_beta ]
                                                                 ->get_irrep() );
                        int IRdown = Irreps::directProd( IR, Btensors[ theindex + 1 ][ l_beta - l_alpha ][ theindex + 1 - l_beta ]
                                                                 ->get_irrep() );
                        int memSkappa = denS->gKappa( NL + 2, TwoSLdown, ILdown, N1, N2, TwoJ, NR + 2, TwoSRdown, IRdown );
...
void CheMPS2::CHeffNS::addDiagram2a3spin0( const int ikappa, dcomplex * memS, dcomplex * memHeff, CSobject * denS, CTensorOperator **** Ctensors, CTensorOperator **** CTtensors, CTensorF0 **** F0tensors, CTensorF0T **** F0Ttensors, dcomplex * workspace ) {
...
#endif
            {
               int ILdown = Irreps::directProd( IL, F0tensors[ theindex - 1 ][ l_alpha - l_gamma ][ theindex - 1 - l_alpha ]
                                                        ->get_irrep() );
               int IRdown = Irreps::directProd( IR, Ctensors[ theindex + 1 ][ l_alpha - l_gamma ][ theindex + 1 - l_alpha ]
                                                        ->get_irrep() );
...
#endif
            {
               int ILdown = Irreps::directProd( IL, F0Ttensors[ theindex - 1 ][ l_gamma - l_alpha ][ theindex - 1 - l_gamma ]
                                                        ->get_irrep() );
               int IRdown = Irreps::directProd( IR, CTtensors[ theindex + 1 ][ l_gamma - l_alpha ][ theindex + 1 - l_gamma ]
                                                        ->get_irrep() );
               int memSkappa = denS->gKappa( NL, TwoSL, ILdown, N1, N2, TwoJ, NR, TwoSR, IRdown );
...
#endif
            {
               int ILdown = Irreps::directProd( IL, Ctensors[ theindex - 1 ][ l_beta - l_delta ][ l_delta - theindex ]
                                                        ->get_irrep() );
               int IRdown = Irreps::directProd( IR, F0tensors[ theindex + 1 ][ l_beta - l_delta ][ l_delta - theindex - 2 ]
                                                        ->get_irrep() );
               int memSkappa = denS->gKappa( NL, TwoSL, ILdown, N1, N2, TwoJ, NR, TwoSR, IRdown );
...
#endif
            {
               int ILdown = Irreps::directProd( IL, CTtensors[ theindex - 1 ][ l_delta - l_beta ][ l_beta - theindex ]
                                                        ->get_irrep() );
               int IRdown = Irreps::directProd( IR, F0Ttensors[ theindex + 1 ][ l_delta - l_beta ][ l_beta - theindex - 2 ]
                                                        ->get_irrep() );
               int memSkappa = denS->gKappa( NL, TwoSL, ILdown, N1, N2, TwoJ, NR, TwoSR, IRdown );
...
void CheMPS2::CHeffNS::addDiagram2a3spin1( const int ikappa, dcomplex * memS, dcomplex * memHeff, CSobject * denS, CTensorOperator **** Dtensors, CTensorOperator **** DTtensors, CTensorF1 **** F1tensors, CTensorF1T **** F1Ttensors, dcomplex * workspace ) {
...
#endif
                     {
                        int ILdown = Irreps::directProd( IL, F1tensors[ theindex - 1 ][ l_alpha - l_gamma ][ theindex - 1 - l_alpha ]
                                                                 ->get_irrep() );
                        int IRdown = Irreps::directProd( IR, Dtensors[ theindex + 1 ][ l_alpha - l_gamma ][ theindex + 1 - l_alpha ]
                                                                 ->get_irrep() );
...
#endif
                     {
                        int ILdown = Irreps::directProd( IL, F1Ttensors[ theindex - 1 ][ l_gamma - l_alpha ][ theindex - 1 - l_gamma ]
                                                                 ->get_irrep() );
                        int IRdown = Irreps::directProd( IR, DTtensors[ theindex + 1 ][ l_gamma - l_alpha ][ theindex + 1 - l_gamma ]
                                                                 ->get_irrep() );
                        int memSkappa = denS->gKappa( NL, TwoSLdown, ILdown, N1, N2, TwoJ, NR, TwoSRdown, IRdown );
                  ...
                  for ( int l_beta = l_delta + 1; l_beta < Prob->gL(); l_beta++ ) {
...
#endif
                     {
                        ...
                        if ( memSkappa != -1 ) {
                           ...
                           dcomplex beta = 0.0;
                           zgemm_( &notrans, &notrans, &dimL, &dimRdown, &dimLdown, &prefactor, ptr, &dimL, memS + denS->gKappa2index( memSkappa ), &dimLdown, &beta, workspace, &dimL );
...
void CheMPS2::CHeffNS::addDiagram2b1and2b2( const int ikappa, dcomplex * memS, dcomplex * memHeff, CSobject * denS, CTensorOperator * Atensor, CTensorOperator * ATtensor ) {
   ...
   if ( N1 == 0 ) {
      ...
      if ( dimLdown > 0 ) {
         ...
         if ( memSkappa != -1 ) {
            ...
            dcomplex beta     = 1.0;
            zgemm_( &notrans, &notrans, &dimLup, &dimR, &dimLdown, &alpha, BlockA, &dimLup, memS + denS->gKappa2index( memSkappa ), &dimLdown, &beta, memHeff, &dimLup );
         }
      }
   }
...
void CheMPS2::CHeffNS::addDiagram2c1and2c2( const int ikappa, dcomplex * memS, dcomplex * memHeff, CSobject * denS, CTensorOperator * Atensor, CTensorOperator * ATtensor ) {
...
void CheMPS2::CHeffNS::addDiagram2dall( const int ikappa, dcomplex * memS, dcomplex * memHeff, CSobject * denS ) {

   const int N1       = denS->gN1( ikappa );
   const int N2       = denS->gN2( ikappa );
   const int theindex = denS->gIndex();
   int size           = bk_down->gCurrentDim( theindex, denS->gNL( ikappa ), denS->gTwoSL( ikappa ), denS->gIL( ikappa ) ) * bk_down->gCurrentDim( theindex + 2, denS->gNR( ikappa ), denS->gTwoSR( ikappa ), denS->gIR( ikappa ) );
   int inc            = 1;
   ...
   if ( ( N1 == 2 ) && ( N2 == 0 ) ) { //2d1
      ...
      if ( memSkappa != -1 ) {
         ...
         zaxpy_( &size, &factor, memS + denS->gKappa2index( memSkappa ), &inc, memHeff, &inc );
      }
   }
   ...
   if ( ( N1 == 2 ) && ( N2 == 2 ) ) { //2d3a
      ...
      zaxpy_( &size, &factor, memS + denS->gKappa2index( ikappa ), &inc, memHeff, &inc );
   }
...
void CheMPS2::CHeffNS::addDiagram2e1and2e2( const int ikappa, dcomplex * memS, dcomplex * memHeff, CSobject * denS, CTensorOperator * Atensor, CTensorOperator * ATtensor ) {
   ...
   if ( N1 == 2 ) {
      ...
      if ( memSkappa != -1 ) {
         ...
         dcomplex alpha    = sqrt( 2.0 );
         dcomplex beta     = 1.0;
         zgemm_( &notrans, &cotrans, &dimL, &dimRup, &dimRdown, &alpha, memS + denS->gKappa2index( memSkappa ), &dimL, BlockA, &dimRup, &beta, memHeff, &dimL );
      }
   }
...
void CheMPS2::CHeffNS::addDiagram2f1and2f2( const int ikappa, dcomplex * memS, dcomplex * memHeff, CSobject * denS, CTensorOperator * Atensor, CTensorOperator * ATtensor ) {
...
void CheMPS2::CHeffNS::addDiagram2b3spin0( const int ikappa, dcomplex * memS, dcomplex * memHeff, CSobject * denS, CTensorOperator * CTtensor ) {
   ...
   if ( N1 != 0 ) {
      ...
      int dimR  = bk_down->gCurrentDim( theindex + 2, denS->gNR( ikappa ), denS->gTwoSR( ikappa ), denS->gIR( ikappa ) );

      dcomplex * Cblock = CTtensor->gStorage( NL, TwoSL, IL, NL, TwoSL, IL );
      dcomplex alpha    = ( ( N1 == 2 ) ? 1.0 : 0.5 ) * sqrt( 2.0 );
      dcomplex beta     = 1.0;
      zgemm_( &notrans, &notrans, &dimLU, &dimR, &dimLD, &alpha, Cblock, &dimLU, memS + denS->gKappa2index( ikappa ), &dimLD, &beta, memHeff, &dimLU );
   }
}

void CheMPS2::CHeffNS::addDiagram2c3spin0( const int ikappa, dcomplex * memS, dcomplex * memHeff, CSobject * denS, CTensorOperator * CTtensor ) {
...
void CheMPS2::CHeffNS::addDiagram2e3spin0( const int ikappa, dcomplex * memS, dcomplex * memHeff, CSobject * denS, CTensorOperator * CTtensor ) {
   ...
   if ( N1 != 0 ) {
      ...
      int dimL  = bk_down->gCurrentDim( theindex, denS->gNL( ikappa ), denS->gTwoSL( ikappa ), denS->gIL( ikappa ) );

      dcomplex * Cblock = CTtensor->gStorage( NR, TwoSR, IR, NR, TwoSR, IR );
      dcomplex alpha    = ( ( N1 == 2 ) ? 1.0 : 0.5 ) * sqrt( 2.0 );
      dcomplex beta     = 1.0;
      zgemm_( &notrans, &cotrans, &dimL, &dimRU, &dimRD, &alpha, memS + denS->gKappa2index( ikappa ), &dimL, Cblock, &dimRU, &beta, memHeff, &dimL );
   }
}

void CheMPS2::CHeffNS::addDiagram2f3spin0( const int ikappa, dcomplex * memS, dcomplex * memHeff, CSobject * denS, CTensorOperator * CTtensor ) {
...
void CheMPS2::CHeffNS::addDiagram2b3spin1( const int ikappa, dcomplex * memS, dcomplex * memHeff, CSobject * denS, CTensorOperator * DTtensor ) {
            ...
            for ( int TwoJdown = TwoJstart; TwoJdown <= 1 + TwoS2; TwoJdown += 2 ) {
               if ( abs( TwoSLdown - TwoSR ) <= TwoJdown ) {
                  ...
                  if ( memSkappa != -1 ) {
                     ...
                     dcomplex beta  = 1.0;
                     zgemm_( &notrans, &notrans, &dimLup, &dimR, &dimLdown, &alpha, Dblock, &dimLup, memS + denS->gKappa2index( memSkappa ), &dimLdown, &beta, memHeff, &dimLup );
                  }
               }
            }
         }
...
void CheMPS2::CHeffNS::addDiagram2c3spin1( const int ikappa, dcomplex * memS, dcomplex * memHeff, CSobject * denS, CTensorOperator * DTtensor ) {
...
void CheMPS2::CHeffNS::addDiagram2e3spin1( const int ikappa, dcomplex * memS, dcomplex * memHeff, CSobject * denS, CTensorOperator * DTtensor ) {
            ...
            for ( int TwoJdown = TwoJstart; TwoJdown <= 1 + TwoS2; TwoJdown += 2 ) {
               if ( abs( TwoSL - TwoSRdown ) <= TwoJdown ) {
                  ...
                  if ( memSkappa != -1 ) {
                     ...
                     dcomplex beta  = 1.0;
                     zgemm_( &notrans, &cotrans, &dimL, &dimRup, &dimRdown, &alpha, memS + denS->gKappa2index( memSkappa ), &dimL, Dblock, &dimRup, &beta, memHeff, &dimL );
                  }
               }
            }
         }
...
void CheMPS2::CHeffNS::addDiagram2f3spin1( const int ikappa, dcomplex * memS, dcomplex * memHeff, CSobject * denS, CTensorOperator * DTtensor ) {
